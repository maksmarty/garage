...
//         }
    ...
    public function postAdd(){
        ...
        if (Input::hasFile('image')) {
            //$rules['image[]'] = 'required|mimes:png,gif,jpeg,jpg';//|max:20000
        }

        //echo '<pre>';print_r(Input::file('image'));print_r(Input::all());die('======Debugging=======');
...
//    }
    ...
    public function getLoadView($formType, $data, $section = NULL) {
        $html = View::make('showroom::lists', $data)->render();
    ...
    public function getPhone() {
        ...
        $showrooms = Showroom::where('is_processed','!=',1)->get();
//echo '<pre>';print_r($showrooms);die('======Debugging=======');
        foreach( $showrooms as $showroom ){

            if( !empty($showroom->showroom_car_id) && !empty($showroom->contact) ){

                $phoneStr = trim(preg_replace('/[-]+/', ' ', preg_replace('/[\s]+/', '-', trim(preg_replace('/[^\d-\s]+/', '', $showroom->contact)))));

//                $showroom = Showroom::find($showroom->showroom_car_id);
//                $showroom->phone       = $phoneStr;
//                $showroom->save();
//
//                unset($showroom);
